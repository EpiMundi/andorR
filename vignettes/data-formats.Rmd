...
vignette: >
  %\VignetteIndexEntry{Data Formats for andorR}
...
## 1. The Hierarchical (YAML) Format
...
* Optional attributes like `rule` and `question` can be added to any node.

An excerpt from a YAML file (`ethical_tree.yml`) looks like this:
...
yaml_path <- system.file("extdata", "ethical_tree.yml", package = "andorR")
...
* Optional attributes: `question`, `rule`.
...
| :--- | :--- | :--- | :--- |
| **Hierarchical** | Manual creation, readability, version control (git) | Indented YAML file with a `nodes` key | `load_tree_yaml()` |
